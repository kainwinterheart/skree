...
-I contrib-build/usr/lib -std=c++11 -fstack-protector-all -Wno-expansion-to-defined

# LDFLAGS = -L /usr/local/lib -fsanitize=address,undefined,safe-stack -L contrib-build/usr/local/lib -L contrib-build/usr/lib
LDFLAGS = -L /usr/local/lib -L contrib-build/usr/local/lib -L contrib-build/usr/lib
...
LIBS = -lpthread -lc -lz -lkyotocabinet -lstdc++ -lm -lyaml-cpp -ldl
